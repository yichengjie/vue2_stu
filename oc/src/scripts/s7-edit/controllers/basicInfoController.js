   ...
   controllers.controller('BasicInfoCtrl',['$scope','HttpOperService','FormData','DEFAULT_SERVICETYPE','FormEditStatusServcie',function($scope,HttpOperService,FormData,DEFAULT_SERVICETYPE,FormEditStatusServcie){
		...
		var selectize = initNewVersionServcieChoose(function(id){
			if(id&&id.length>0){
				...
				var serviceGroupDescription = curItem.serviceGroupDescription || '空' ;
				if(serviceGroupDescription.length==0){
					serviceGroupDescription = '空' ;
				}
				var subGroupDescription = curItem.subGroupDescription || '空' ;
				if(subGroupDescription.length==0){
					subGroupDescription = '空' ;
				}
		...
		$scope.subGroupQuery = function(showStr,serviceGroup){
			...
			var clearOption = {FormData,initServiceType} ;
			$scope.lastGroupList = [] ;
