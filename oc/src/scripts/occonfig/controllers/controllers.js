...
//	$scope.subGroupTable = {"showStatus":"false"};
	...
	$scope.serviceTypeList=[ {"name":"运价相关服务","value":"F"},{"name":"商品相关服务","value":"M"},
	     				   ...
	     				   {"name":"免费行李服务","value":"A"},{"name":"逾重行李服务","value":"C"},
	    				    {"name":"预付费行李服务","value":"P"},{"name":"禁运行李服务","value":"E"},
	    				    {"name":"随携行李服务","value":"B"}];
	
	$scope.rficList = [ {"name":"选择","value":""},{"name":"航空运输","value":"A"},{"name":"地面交通/非航空服务","value":"B"},
	...
	$scope.submitPage = function(){
		...
		$.showTuiConfirmDialog('保存?', function() {
			...
			if(ngFlag=="true"){
				...
				promise.then(function(retData){
					if(retData.flag==true||retData.flag=='true'){
						...
						if(retData.errMsg!=undefined &&　retData.errMsg !=""){
							...
							$.bootstrapGrowl('系统异常，保存不成功',growlConfig.danger) ; 
						}
					}
				},function(err){
				...
				$.bootstrapGrowl(ngFlag,growlConfig.danger) ; 
			}
		});
	};
...
//		$scope.showDescFlag=true;
		...
		showTable();
		$scope.data.selfSubCode=l.serviceSubCode;
		$scope.data.selfGroupCode=l.attributesGroup;
		$scope.data.selfGroupDesc=l.serviceGroupDescription;
		$scope.data.selfSubGroupCode=l.attributesSubgroup;
		$scope.data.selfSubGroupDesc=l.subGroupDescription;
		$scope.data.commercialName=l.commercialName;
		$scope.data.rfic = l.rfic;
		...
		promise.then(function(retData){
			if(retData.flag==true||retData.flag=='true'){
				...
				if(retData.errorMsg!=undefined && retData.errorMsg!=""){
					...
					$.bootstrapGrowl("获取信息失败！",growlConfig.danger) ; 
				}
	...
	$scope.deleteS5= function(l){
		var serverURL =contextPath+ "/occonfig/checkDelete.action" ;
		var promise = HttpOperService.post4JSONData(serverURL,l);
		promise.then(function(resultData){
			if(resultData.flag==true||resultData.flag=='true'){
				$.showTuiConfirmDialog('删除?', function() {
					serverURL =contextPath+ "/occonfig/deleteConfig.action" ;
					promise = HttpOperService.post4JSONData(serverURL,l);
					promise.then(function(retData){
						if(retData.flag==true||retData.flag=='true'){
							//console.info("删除成功");
							...
							if(retData.errorMsg!=undefined && ""!=retData.errorMsg){
								...
								$.bootstrapGrowl("系统异常，删除不成功！",growlConfig.danger) ; 
							}
				...
				if(resultData.errorMsg!=undefined && ""!=resultData.errorMsg){
					$.bootstrapGrowl(resultData.errorMsg,growlConfig.danger) ; 
				}else{
					$.bootstrapGrowl("系统异常!") ; 
				}
			}
	...
	$scope.checkSelfSubCode =  function(){
		...
		if(selfSubCode!=undefined && selfSubCode.length==3 && selfSubCode!=$scope.data.oldSubCode && $scope.data.errMsgSelfSubCode==""){
			...
			promise.then(function(retData){
	    		if(retData.flag=='true'){
	    				if(retData.existFlag!='true'){
	    					$scope.data.errMsgSelfSubCode = "该SUBCODE符合行业规范或已建立，请取消自定义";
						}else{
							$scope.data.errMsgSelfSubCode = "";
						}
	    		}else{
	    			if(retData.errorMsg!=undefined && ""!=retData.errorMsg){
						$scope.data.errMsgSelfSubCode = retData.errorMsg;
					}else{
						$scope.data.errMsgSelfSubCode = "网络异常";
					}
	    		}
	    	}) ;
		}
	...
	var validate = function(){
		...
		if($scope.showHideTableFlag){
			...
			$scope.data.errMsgCommercialName==""){
				...
				if($scope.data.errMsgSelfSubCode!="" ){
					errMg = errMg+$scope.data.errMsgSelfSubCode+"<br>";
				}
				if($scope.data.errMsgSelfGroupCode!="" ){
					errMg = errMg+ $scope.data.errMsgSelfGroupCode+"<br>";
				}
				if($scope.data.errMsgSelfGroupDesc!="" ){
					errMg = errMg+ $scope.data.errMsgSelfGroupDesc+"<br>";
				}
				if($scope.data.errMsgSelfSubGroupCode!="" ){
					errMg = errMg+ $scope.data.errMsgSelfSubGroupCode+"<br>";
				}
				if($scope.data.errMsgCommercialName!="" ){
					errMg = errMg+ $scope.data.errMsgCommercialName+"<br>";
				}
				return errMg;
			}
		}else{
			...
			if($scope.data.errMsgBySubCode==undefined || $scope.data.errMsgBySubCode==""){
				...
				return $scope.data.errMsgBySubCode;
			}
		}
	};
	...
	var validateSelfGroupCode = function(){
		...
		if(selfGroupCode!=undefined && selfGroupCode!=""){
			 if(!util.islettersOrNumber(selfGroupCode)){
				 errMsg="请输入正确的GROUP";
            }else if(selfGroupCode.length<2){
            	errMsg="GROUP字符长度需大于2";
            }
		}else{
			errMsg="GROUP必填";
		}
	...
	var validateSelfSubGroupCode = function(){
		...
		if(selfSubGroupCode!=undefined && selfSubGroupCode!=""){
            ...
            }else if(selfSubGroupCode.length<2){
            	errMsg="SUBGROUP字符长度需大于2";
            }
		}
